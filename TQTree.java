...
public class TQTree {
    ...
    public void play(Scanner input)
    {
        ...
        TQNode current = root;
        while(current.getYesChild()!=null && current.getNoChild()!=null)
        {
        	if(response.equalsIgnoreCase("y") || response.equalsIgnoreCase("yes"))
        	{
        		current = current.yesChild;
        		if(current.getYesChild()!=null && current.getNoChild()!=null)
        		{
        ...
        response = input.nextLine();
        gameEnd(input, response, current);
    ...
    // 'next()'
    public void askQuestion(TQNode last)
    {
    	System.out.println("Is it a " + last.getData() + "?");    	
    }
    
    public void gameEnd(Scanner input, String response, TQNode current)
    {
    	String answer;
    	String question;
    	String alt_answer;
    	...
    	else
    	{
    		...
    		response = input.nextLine();
    		answer =  response;
    		System.out.println("Give me a question that would distinguish " + answer + " from " + current.getData());
    		response = input.nextLine();
    		question = response;
    		System.out.println("And would the answer to the question for " + answer + " be yes or no?");
    		response = input.nextLine();
    		alt_answer = current.getData();
    		if(response.equals("yes"))
    		{
    			current.data = question;
    			TQNode no = new TQNode(alt_answer);
    			current.setNoChild(no);
    			TQNode yes = new TQNode(answer);
    			current.setYesChild(yes);
    		}
    		else
    		{
    			current.data = question;
    			TQNode no = new TQNode(answer);
    			current.setNoChild(no);
    			TQNode yes = new TQNode(alt_answer);
    			current.setYesChild(yes);	
    		}
    	}
    }
    ...
    private void printTree(PrintWriter writer, TQNode root) {
    	LinkedList<TQNode> first = new LinkedList<>();
    	LinkedList<TQNode> second = new LinkedList<>();
    	if(root==null)
    	{
    		return;
    	}
        ...
        while(!first.isEmpty())
        {
        	TQNode current = first.getFirst();
        	if(current.getNoChild()!=null)
        	{
        		first.addLast(current.getNoChild());
        	}
        	if(current.getYesChild()!=null)
        	{
        		first.addLast(current.getYesChild());
        	}
        	first.getFirst().setIndex(index);
        ...
        {
        	if(second.getFirst().getNoChild()!=null && second.getFirst().getYesChild()!=null)
        	{
        		writer.println(second.getFirst().getIndex() + ": '" + second.getFirst().getData() + "' no:(" + second.getFirst().getNoChild().getIndex() + ") yes:(" + second.getFirst().getYesChild().getIndex() + ")");
        	}
        	else
        	{
        		writer.println(second.getFirst().getIndex() + ": '" + second.getFirst().getData() + "' no:(null) yes:(null)");
        	}
        	second.removeFirst();
        }
    	
    	//System.err.println("The printTree method has not yet been implemented");
    }
